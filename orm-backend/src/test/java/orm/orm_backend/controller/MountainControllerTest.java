...
public class MountainControllerTest {
    ...
    void setUp() {
        ...
        lenient().when(searchMountain1.getId()).thenReturn(mountain1Id);
        lenient().when(searchMountain1.getName()).thenReturn(mountain1Name);
        lenient().when(searchMountain1.getCode()).thenReturn(mountain1Code);

        lenient().when(searchMountain2.getId()).thenReturn(mountain2Id);
        lenient().when(searchMountain2.getName()).thenReturn(mountain2Name);
        lenient().when(searchMountain2.getCode()).thenReturn(mountain2Code);
    }
    ...
    void show100() throws Exception {
        ...
        given(mountainService.get100Mountains()).willReturn(List.of(searchMountain1, searchMountain2));
    ...
    void getAllMountainsBySearch() throws Exception {
        ...
        given(mountainService.getMountainsBySearch(name)).willReturn(List.of(searchMountain1, searchMountain2));

        mockMvc.perform(get(url + api + "?name=" + name))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].id").value(searchMountain1.getId()))
                .andExpect(jsonPath("$[0].name").value(searchMountain1.getName()))
                .andExpect(jsonPath("$[0].code").value(searchMountain1.getCode()))
                .andExpect(jsonPath("$[1].id").value(searchMountain2.getId()))
                .andExpect(jsonPath("$[1].name").value(searchMountain2.getName()))
                .andExpect(jsonPath("$[1].code").value(searchMountain2.getCode()));
    }
