...
# 파일 경로 설정
unique_coordinates_path = 'C:/Users/SSAFY/Desktop/ORM_data_parsing/240808/release.csv'
entrypoints_path = 'C:/Users/SSAFY/Desktop/ORM_data_parsing/240808/highest_elevation_data.csv'
output_file_path = 'C:/Users/SSAFY/Desktop/ORM_data_parsing/240808/release_updated.csv'
...
for _, entry_row in entrypoints_df.iterrows():
    ...
    mntn_code = entry_row['MNTN_CODE']
    
    # Latitude와 Longitude가 유효한지 확인
    if pd.isnull(entry_latitude) or pd.isnull(entry_longitude):
        continue
    
    # 기존 'info'에 '정상'이라고 써있는 정보를 빈 값으로 변경
    unique_coordinates_df.loc[(unique_coordinates_df['MNTN_CODE'] == mntn_code) & (unique_coordinates_df['info'] == '정상'), 'info'] = ''
    
    closest_idx = find_closest_coordinates(entry_row, unique_coordinates_df)
    if closest_idx != -1:
        unique_coordinates_df.at[closest_idx, 'info'] = entry_row['Entry_Point']
